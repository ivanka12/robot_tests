...
Створити тендер
  ...
  ${status}=  Set Variable If  'open' in '${MODE}' or 'twostage' in '${MODE}'  active.tendering  ${EMPTY}
...
Подати цінову пропозицію
  ...
  Log  ${reply}
  ${status}=  Set Variable If  '${MODE}'=='openeu' or '${MODE}'=='twostage'  pending  active
...
Редагувати угоду
  [Arguments]  ${username}  ${tender_uaid}  ${contract_index}  ${field}  ${fieldvalue}
  ${tender}=  openprocurement_client.Пошук тендера по ідентифікатору  ${username}  ${tender_uaid}
  ...
  Set_To_Object  ${contract.data}  ${field}  ${fieldvalue}
