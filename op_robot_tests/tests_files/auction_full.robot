...
Можливість проведення 2 го раунду аукціону для першого учасника
  ...
  Click Button  id=clear-bid-button
  Поставити мінімально можливу ставку
  Дочекатись учасником закінчення стадії ставок
  Перевірити чи ставка була прийнята
...
Поставити мінімально можливу ставку
  ...
  Wait Until Element Is Visible    id=max_bid_amount_price
  ...
  ${last_amount}=  addition  ${last_amount}  ${value}
  Поставити ставку   ${last_amount}   Заявку прийнято


Поставити ставку меншу від мінімальної на ${extra_amount} грн
  Wait Until Page Contains Element  id=max_bid_amount_price
  ${last_amount}=  Wait Until Keyword Succeeds  30 times  5s  Get Text  id=max_bid_amount_price
  ${last_amount}=  convert_amount_string_to_float  ${last_amount}
...
Відмінитити ставку
  Click Button             id=cancel-bid-button
